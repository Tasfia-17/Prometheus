"""Minimal directed graph — zero external dependencies.

Implements only what OpenAPIGraph and SubgraphRetriever need:
add_node, add_edge, has_node, nodes(data), edges(data), number_of_nodes/edges.
"""
    ...
    def __call__(self, node=None, data: bool = False):
        ...
        for source, targets in self._adj.items():
            for target, attrs in targets.items():
                result.append((source, target, attrs) if data else (source, target))
        return result
